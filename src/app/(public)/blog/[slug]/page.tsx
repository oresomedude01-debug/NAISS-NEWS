/**
 * Single Blog Post Page — Premium Editorial Reading Experience
 * Full-width hero, optimized reading width, share buttons, related posts
...
import { getPostBySlug, getAllPostSlugs } from '@/lib/fetcher';
    ...
    <>
      ...
      <div className="pb-20 md:pb-0">
        ...
        <div className="container-page pt-6" id="post-breadcrumb">
          <nav className="flex items-center gap-1.5 text-xs text-surface-500 dark:text-surface-500 overflow-x-auto scrollbar-none">
            <Link href="/" className="hover:text-brand-600 dark:hover:text-brand-400 transition-colors shrink-0">
              Home
            </Link>
            <ChevronRight className="w-3 h-3 shrink-0 opacity-40" />
            <Link href="/blog" className="hover:text-brand-600 dark:hover:text-brand-400 transition-colors shrink-0">
              Blog
            </Link>
            <ChevronRight className="w-3 h-3 shrink-0 opacity-40" />
            <Link
            ...
            <ChevronRight className="w-3 h-3 shrink-0 opacity-40" />
            <span className="text-surface-900 dark:text-surface-300 truncate">
              {post.title}
            </span>
          </nav>
        </div>
        ...
        <article className="container-narrow pt-8 md:pt-12" id="post-article">
          <header className="mb-8 md:mb-10">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5 pb-8 border-b border-surface-200 dark:border-surface-800">
              ...
              <div className="flex items-center gap-2">
                <button className="btn-ghost p-2.5 rounded-xl" aria-label="Bookmark">
                  <Bookmark className="w-4 h-4" />
                </button>
                <button className="btn-ghost p-2.5 rounded-xl" aria-label="Like">
                  <Heart className="w-4 h-4" />
                </button>
                <button className="btn-ghost p-2.5 rounded-xl" aria-label="Share">
          ...
          {post.image?.asset?.url && (
            <div className="mb-10 md:mb-14 -mx-4 sm:mx-0" id="post-image">
              ...
              {post.image.alt && (
                <p className="text-center text-xs text-surface-400 mt-3 italic px-4">
                  {post.image.alt}
                </p>
              )}
            </div>
          ...
          <div className="max-w-[680px] mx-auto mt-10">
            <Link
              href="/blog"
              className="btn-outline rounded-xl text-sm inline-flex"
              id="back-to-blog"
            >
